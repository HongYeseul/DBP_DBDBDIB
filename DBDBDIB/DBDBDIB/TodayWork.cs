...
namespace DBDBDIB
{
    public partial class TodayWork : Form
    {
        ...
        public void SettingValue() //gridview 전부 출력
        {
            ...
            string query = "Select id AS '번호',등록자,날짜,소속,업무종류,업무내용 FROM 일일업무등록,업무마스터 WHERE 업무마스터id=업무번호";
        ...
        {
            //등록 창 불러오기
            TodayWorkSaveChange todayWorkSaveChange = new TodayWorkSaveChange("등록");
        ...
        private void buttonChange_Click(object sender, EventArgs e) //수정버튼
        {
            ...
            //수정 창 불러오기
            TodayWorkSaveChange todayWorkSaveChange = new TodayWorkSaveChange("수정", id);
            todayWorkSaveChange.Show();
        }
        ...
        private void buttonSelcet_Click(object sender, EventArgs e) //검색버튼
        {
            ...
            if (textbox==""&&radioButtonDate.Checked==false)
            {
                ...
                query = "Select id AS '번호',등록자,날짜,소속,업무종류,업무내용 FROM 일일업무등록,업무마스터 WHERE 업무마스터id=업무번호"; //전체 검색
            ...
            switch (radiobuttonNow)
            {
                case "날짜": //날짜로 select로 쿼리
                    ...
                    query = "SELECT id AS '번호',등록자,날짜,소속,업무종류,업무내용 FROM 일일업무등록,업무마스터 WHERE 업무마스터id=업무번호 AND 날짜='" + date + "'";


                    break;
                case "키워드": //키워드로 select 쿼리 **
                    //소분류에서 키워드 검색 *** 중분류,대분류 검색해야할 수 도 있음 *** 수정 주의
                    query = "SELECT id AS '번호',등록자,날짜,소속,업무종류,업무내용 FROM 일일업무등록,업무마스터 WHERE 업무마스터id=업무번호 AND 업무내용 LIKE '%" + textbox +"%'";

                    break;
                case "등록자": //등록자 기반 검색 select
                    //유저 이름으로 검색
                    query = "SELECT id AS '번호',등록자,날짜,소속,업무종류,업무내용 FROM 일일업무등록,업무마스터 WHERE 업무마스터id=업무번호 AND 등록자 LIKE '%" + textbox + "%'";
